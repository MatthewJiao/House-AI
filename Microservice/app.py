...
#condition prediction setup start
dense_3_nn = tf.keras.models.load_model('model_4_covid_e.model')

symptoms = pd.read_csv('Symptoms.csv')
symptoms = symptoms['0'].to_list()
symptoms = [item.lower() for item in symptoms]


conditions = pd.read_csv('Conditions.csv')
conditions = conditions['Condition'].to_list()
#condition prediction setup end
...
def nl(x):
    return get_symptoms(x)

def pred(x):
    oneHotEncoded = [0]*809
    for i in range(len(x)):
        index = symptoms.index(x[i].lower())
        oneHotEncoded[index] = 1

    predictions = dense_3_nn.predict([oneHotEncoded])
    lst1 = -np.sort(-predictions[0])
    lst2 = np.argsort(-predictions[0])
...
def index1(features):
    try:
        ...
        '''test = [0]*809
                                ...
                                test[405] = 1
                                test[56] = 1    
                                predictions = dense_3_nn.predict([test])
                                prediction = np.argmax(predictions[0])
                                prediction = np.array2string(prediction, precision=2, separator=',',suppress_small=True)'''


        symptomLst = nl(features)
        conditionLst = pred(symptomLst)
        symptomLst = ','.join(symptomLst)
        data = conditionLst + 'QQQ' + symptomLst
        return data
    except:
        return "Error"
